...
async function getMissedSearchRank(dates?: Array<string>, limit = 50): Promise<Array<{ count: number; tag: string; }>> {
    const r = await client.query({
        query:        `SELECT arrayMap(x -> lower(x), tags) as tags, COUNT(*) as count FROM missed_searches WHERE ${dates ? "date IN ({dates:Array(Date)}) AND " : ""}length(tags) = 1 GROUP BY tags ORDER BY count DESC LIMIT {limit:UInt64}`,
